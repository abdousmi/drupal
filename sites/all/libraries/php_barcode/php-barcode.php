...
function barcode_outimage($text, $bars, $scale = 1, $mode = "png", $total_y = 0, $space = '', $filename = NULL){
  $bar_color=Array(0,0,0);
  $bg_color=Array(255,255,255);
  $text_color=Array(0,0,0);
  $font_loc=dirname(__FILE__)."/"."DroidSans.ttf";
  header("Content-type: image/". $mode);
    ...
    $crop = imagecreatetruecolor($total_x, $total_y - 12);

    @imagefilledrectangle($crop, 0, 0, $total_x, $total_y - 12, $col_bg);

    imagecopy($crop, $im, 0, 0, 0, 12, $total_x, $total_y);
    /* output the image */
    $mode=strtolower($mode);
    if ($mode=='jpg' || $mode=='jpeg'){
	@imagejpeg($crop, $filename);
    } else if ($mode=='gif'){
	@imagegif($crop, $filename);
    } else {
...
function barcode_print($code, $encoding="ANY", $scale = 2 ,$mode = "png", $filename = NULL ){
    ...
    else barcode_outimage($bars['text'],$bars['bars'],$scale, $mode, 0, '', $filename);
    return $bars;
}
